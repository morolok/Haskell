...
nHojas (N _ ai ad) = (nHojas ai) + (nHojas ad)
...
nNodos (N _ ai ad) = 1 + (nNodos ai) + (nNodos ad)
...
profundidad (N _ ai ad) = 1 + max (profundidad ai) (profundidad ad)
...
anadeHojas (N v ai ad) x y = N v (anadeHojas ai x y) (anadeHojas ad x y)
...
takeArbol n (N x ai ad) = (N x (takeArbol (n-1) ai) (takeArbol (n-1) ad))
...
mapArbol f (N x ai ad) = (N (f x) (mapArbol f ai) (mapArbol f ad))
...
valorB (Conj ai ad) = (valorB ai) && (valorB ad)

valorB (Disy ai ad) = (valorB ai) || (valorB ad)

valorB (Neg (HB b)) = not b
...
ramifica (NG x as) a2 p
    | p x = NG x ([ramifica a a2 p | a <- as]++[a2])
    | otherwise = NG x (map (\a -> ramifica a a2 p) as)
...
nHojasG (NG _ as) = sum [nHojasG a | a <- as]
...
profundidadG (NG _ as)
    | null as = 0
    | otherwise = 1 + maximum (map profundidadG as)
...
bin2gen (N v ai ad) = NG v [bin2gen ai, bin2gen ad]

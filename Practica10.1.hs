...
-- los multiconjuntos utilizando los diccionarios vistos en clase.
-- http://www.cs.us.es/~jalonso/cursos/i1m-19/doc/manual-Data.Map.html#informaci%C3%B3n-sobre-diccionarios
...
listaAmc xs = listaAmcAux xs (vacio)

listaAmcAux [] m = m

listaAmcAux (x:xs) m = listaAmcAux xs (inserta x m)

listaAmc2 xs = M.fromListWith (+) (zip xs (repeat 1))
